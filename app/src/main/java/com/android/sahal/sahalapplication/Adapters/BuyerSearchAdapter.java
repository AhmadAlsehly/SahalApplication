...
public class BuyerSearchAdapter extends RecyclerView.Adapter<BuyerSearchAdapter.MyViewHolder> {
    ...
    public BuyerSearchAdapter(Context mContext, List<ModuleItem> itemsList) {
...
// to add id
                for (int i = 0; i < MainBuyerActivity.cartList.size(); i++) {
                    ...
                    if (MainBuyerActivity.cartList.get(i).equals(item.getId())) {
                        added = false;
                        Toast.makeText(mContext, "القطعة مضافة مسبقا", Toast.LENGTH_SHORT).show();
                        break;
                    }
                }
                if (added) {
                    MainBuyerActivity.cartList.add(item.getId());
                    Toast.makeText(mContext, "تمت الاضافة للسلة", Toast.LENGTH_SHORT).show();
